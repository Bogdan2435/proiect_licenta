...
def capture_frame_and_control_robot(model, transform, yolo):
    ...
    while True:
        ...
        if state_stop == False and state_left == False and state_right == False:
            ...
            if state_left:
                if time.time() - timp_start_left >= 1:
                    state_left = False
                    continue
            if state_right:
                if time.time() - timp_start_right >= 1:
                    state_right = False
                    continue
            if state_stop == True:
                if time.time() - state_stop_time >= 1:
                    state_stop = False
                    state_stop_time = 0
                    continue
        ...
        if state_crosswalk:
            # speed = 0.30
            # turn_gain = 0.15
            if time.time() - timp_start_cw > 2: # daca au trecut doua secunde de cand a fost detectata trecerea
                state_crosswalk = False
        ...
        if state_stop == False and state_left == False and state_right == False:
            ...
            for label in result.boxes.data:        
                ...
                if label_text == 'stop' and confidence >= 0.4:
                ...
                if label_text == 'crosswalk' and confidence >= 0.4:
                    xmin, ymin, xmax, ymax = box
                    width = xmax - xmin
                    height = ymax - ymin
                    if width * height >= 100:
                ...
                if label_text == 'left' and confidence >= 0.6:
                    xmin, ymin, xmax, ymax = box
                    width = xmax - xmin
                    height = ymax - ymin
                    if width * height >= 700:
                ...
                if label_text == 'right' and confidence >= 0.6:
                    xmin, ymin, xmax, ymax = box
                    width = xmax - xmin
                    height = ymax - ymin
                    if width * height >= 700:
